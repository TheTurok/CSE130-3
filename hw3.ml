...
let sqsum xs =
  let f a x = a + x * x in (* functions recurse at a *)
  let base = 0 in (* multiply while adding the a's*)
...
let pipe fs = (*pipe taking another function taking an input*)
...
let rec clone x n = match n with
  | _ when n <= 0 -> [] (*negative case *)
  | _ -> [x] @ clone x (n-1) (*decrement as you clone*)
...
let padZero l1 l2 = (*compare the lengths to see which list to add zero*)
...
let rec removeZero l = match l with
  ...
  | h::t -> if h = 0 then removeZero t 
      else l  (*keept returning tail until head isn't zero*)
...
(*mulByDigit i (List.rev t) @ [List.hd t + h*i / 10] @ [h*i mod 10]*)
...
let bigMul l1 l2 = 
  let f a x = 
    ...
    let shift = clone 0 a1 in
    let sum = bigAdd a2 (product @ shift) in
      (a1 + 1, sum)
  in
  let base = (0,[]) in
  let args = 
    let (arg1, arg2) = padZero l1 l2 in 
    let l3 = clone arg1 (List.length arg2) in 
      List.rev( List.combine  l3 arg2)
  in
  let (_, res) = List.fold_left f base args in
    res
